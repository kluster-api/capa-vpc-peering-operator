...
PRODUCT_NAME       := capa-vpc-peering-operator
ENFORCE_LICENSE    ?=

GO_PKG   := go.bytebuilders.dev
REPO     := $(notdir $(shell pwd))
BIN      := capa-vpc-peering-operator
COMPRESS ?= no
...
install:
	@cd ../installer; \
	helm upgrade -i capa-vpc-peering-operator charts/capa-vpc-peering-operator --wait \
...
uninstall:
	@cd ../installer; \
	helm uninstall capa-vpc-peering-operator --namespace=$(KUBE_NAMESPACE) || true
...
.PHONY: run
run:
	go run -mod=vendor ./cmd/capa-vpc-peering-operator run \
		--v=3 \
		--secure-port=8443 \
